...
import core.javaBean.ErrorBean;
...
//	public ResponseEntity<ErrorBean> dsadsadsadsa(Exception exception , WebRequest webRequest ) {
	...
	public ResponseEntity<ErrorBean> response(Exception exception) {
		...
		if (exception instanceof DriveException) {
			DriveException couponSystemException = (DriveException) exception;
			if (couponSystemException.getErorType().isLog() == true) {
			...
			System.out.println(exception.getMessage() + "  error message : "
					+ couponSystemException.getErorType().getErrorClientMessage());
